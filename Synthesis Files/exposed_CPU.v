module cpu(
   ...
   output [15:0] instruction_fetch,
   output reg_write_enable_wb,
   output [3:0] reg_write_addr_wb,
   output [15:0] reg_write_data_wb,
   output memRead_mem,
   output memWrite_mem,
   output [15:0] mem_calc_result_mem,
   output [15:0] rt_data_exec,
   output [15:0] reg_write_data_mem,
   output icache_mem_req,
   output icache_hit_signal,
   output dcache_mem_req,
   output dcache_hit_signal
);
...
wire mem_to_reg_mem;            // ← value from memory stage (MEM) to be passed into MW_pipe
